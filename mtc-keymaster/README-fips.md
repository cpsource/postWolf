...
2. Builds a canonical JSON manifest with the git commit, tag, and timestamp
...
Include `fips-manifest-receipt.json` in the release tarball or `.deb` package. This is the artifact that downstream users verify against.
...
- The `fips-manifest-receipt.json` file (shipped with the release)
- `curl` and `jq` (for online verification)
- The CA public key (for offline verification — 32 bytes, typically published on the project website or pinned in the source)
...
./fips-manifest-verify.sh
```

What happens:
1. The script reads `fips-manifest-receipt.json` to get the log index
2. It computes SHA256 of every FIPS source file on your local disk
3. It queries `GET /fips/manifest/<index>` from the server
4. It compares each local hash against the server's logged manifest
5. It queries `GET /fips/manifest/<index>/proof` for a fresh inclusion proof
6. It verifies the inclusion proof (SHA-256 hash chain from leaf to root)
7. It verifies the Ed25519 cosignature on the Merkle tree root
...
./fips-manifest-verify.sh --offline
```

What happens:
1. The script reads `fips-manifest-receipt.json` (contains the manifest, proof, and signature)
2. It computes SHA256 of every FIPS source file on your local disk
3. It recomputes the manifest hash from local files and compares to the receipt's manifest hash
4. It replays the inclusion proof: walks the hash chain from the leaf hash up to the root
5. It verifies the Ed25519 cosignature on the root using the provided CA public key
...
```bash
./fips-manifest-verify.sh 2>&1 | grep MISMATCH
```
...
The receipt file shipped with the kit contains:
...
1. Computes SHA256 of each local source file
2. Compares against the manifest in the receipt
3. Replays the inclusion proof (hash chain from leaf to root)
4. Verifies the Ed25519 cosignature on the root
