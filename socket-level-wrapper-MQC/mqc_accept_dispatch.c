/******************************************************************************
 * File:        mqc_accept_dispatch.c
 * Purpose:     Server-side accept dispatchers — mqc_accept (mode-from-ctx)
 *              and mqc_accept_auto (mode-from-first-frame).
 *
 * Description:
 *   Split out of mqc.c as part of the client/server TU separation that
 *   keeps libcurl / libhiredis off MQC client link lines.  Both dispatchers
 *   are server-only, so this TU is pulled into the link only when the
 *   binary actually accept()s connections.
 ******************************************************************************/
