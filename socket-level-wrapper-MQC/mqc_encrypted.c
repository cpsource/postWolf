 ...
 * Purpose:     Client-side encrypted-identity handshake (mqc_connect_encrypted).
 *
 * Description:
 ...
 *
 *   The server side (mqc_accept_encrypted, mqc_accept_encrypted_continue)
 *   lives in mqc_encrypted_accept.c so the rate-limit + AbuseIPDB chain
 *   stays out of pure-client link lines.
 *
 *   All shared machinery (transcript hash, key schedule, AEAD seal,
 *   Finished frame, JSON parsers, peer verify) lives in mqc_common.c
 *   and is reached via mqc_internal.h.
 *
 * Created:     2026-05-03  (Phase 7 commit 2)
