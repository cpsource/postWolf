...
/*
 * P0 / TODO #9b show-tpm surface — fetch the cosigner-key
 * fingerprint from a DNSSEC-validated TXT record at
 * `_mqc-cosigner.<parent_domain>`.  Per-RR matching: a single RR
 * must carry v=MQC1; role=cosigner; alg=ML-DSA-87;
 * kh=sha3-256:<hex>.  Same load-bearing rule the server-tree
 * `mqc_dnssec_validate_ca_kh` uses for the opposite trust
 * direction.  Mirrors the helper of the same name in
 * mtc-keymaster/server2/c/mtc_dnssec_pin.c — kept parallel so
 * each tree can drift independently.
 */
                                                 ...
                                                 char *out_hex)
{
    ...
    /* Trust-anchor probe: stat() each candidate before
     * registering, since ub_ctx_add_ta_file is lazy and a
     * non-existent path would fail validator init at resolve
     * time even when a valid path was also registered. */
    {
        static const char *const ta_paths[] = {
            "/var/lib/unbound/root.key",
            "/usr/share/dns/root.key",
     ...
     * tokens (v=MQC1, role=cosigner, alg=ML-DSA-87,
     * kh=sha3-256:<hex>).  On the first RR that does, copy the
     * hash to out_hex and return. */
    for (i = 0; result->data[i]; i++) {
