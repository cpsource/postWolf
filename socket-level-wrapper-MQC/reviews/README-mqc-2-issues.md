...
|---|---|---|---|
| 1 | Handshake not length-prefixed | YES — `mqc_read_json_block` reads byte-by-byte, counts braces, no length prefix.  Spec §5.1 explicitly requires length-prefixed framing for "every unit before and after handshake." | **Agree.** Real spec/code mismatch. **DONE 2026-05-03 (commit `45e8390d4`, mqc-2 P1)** — both legs now go through `mqc_write_handshake_frame` / `mqc_read_handshake_frame`. |
| 2 | Brace-count reader is fragile | YES — line 533–534 of `mqc_common.c`: counts `{` and `}` without string-literal awareness.  `{"k":"}"}` desynchronises framing. | **Agree.** **DONE 2026-05-03 (commit `45e8390d4`, mqc-2 P1)** — `mqc_read_json_block` deleted along with its prototype; nothing else to call. |
| 3 | Cosigner TOFU at first contact | YES — already tracked as TODO #9b in `mtc-keymaster/README-bugsandtodo.md` with two design branches at `~/.claude/plans/lets-plan-to-implement-snoopy-rivest.md`. | **Agree.**  Not new; pick up the existing design.  **OPEN** — mqc-2 master plan Phase 5; substantial. |
| 4 | Peer pubkey not verified against `subject_public_key_hash` | YES — `extract_pubkey_from_cert` reads PEM from local files, never SHA-256s it and compares to the cert's hash field. | **Agree, and this is the most serious one.**  Anyone with write access to `~/.TPM/peers/<n>/public_key.pem` can substitute the verifier pubkey while keeping the cert hash unchanged → identity-substitution. **DONE 2026-05-03 (commit `dfd06d187`, mqc-2 P2)** — every PEM-load path in `mqc_peer.c::extract_pubkey_from_cert` plus `mqc_peer_get_cached_pubkey` now SHA-256s the loaded PEM and fail-closes on mismatch. |
| 5 | Revocation drop-and-retry is brittle | YES, real behavior — but the per-(IP, cert_index) throttle from #12 already mitigates the DoS amplification path.  The drop happens once per (peer, cert_index) lifetime, not per connection. | **Partial disagree.**  Worth revisiting but not security-critical.  **DEFERRED** — mqc-2 master plan Phase 6; may never land. |
| 6 | Transcript binds fields, not raw bytes | YES, by design.  Spec §6.0 explicitly defines a structured byte sequence (LABEL ‖ version ‖ mode ‖ SUITE_ID ‖ KEM ‖ cert_index ‖ role). | **Disagree on severity** — the field-based transcript is a TLS-1.3-style design choice, not a bug.  The reviewer's actual concern is doc clarity around the word "frame," which fix #1 will resolve.  **CLOSED** — Phase 1 length-prefixed framing (commit `45e8390d4`) resolved the doc-clarity concern; the field-based hash is preserved as-is.  The reviewer raised the same item again in `README-mqc-4-issues.md` fix #4; recorded there as **TODO #54** in `mtc-keymaster/README-bugsandtodo.md` (LOW priority). |
| 7 | AEAD recv increments seq before auth | YES — `mqc_enc_recv` line 1420 does `(*seq)++` BEFORE `wc_AesGcmDecrypt` at line 1425.  Same pattern at line 1591 in `mqc_read`. | **Agree.**  Trivial fix (move one line).  Spec wording matters even though the connection terminates on auth failure today.  **DONE 2026-05-04 (commit `88b7fadbe`, mqc-2 P3 / mqc-4 #6)** — both `mqc_enc_recv` and `mqc_read` advance the counter only on `wc_AesGcmDecrypt` success; failure logs name the actually-failing sequence. |
| 8 | Docs say encrypted is stub but code is real | YES — three stale comments in `mqc_internal.h:7`, `mqc.c:15`, `mqc.c:27`. | **Agree.**  Trivial cleanup. **DONE 2026-05-04 (commit `627d28bab`)** — all three comments rewritten to point at the actual ship commit (`a287aa8d0`); `mqc_accept_auto` description updated to the current length-prefixed dispatch. |
| 9 | Auto-detect uses MSG_PEEK + strstr | YES, current behavior. | **Partial agree.**  Architecturally inelegant but the strict-parser fallback catches false positives.  Fix #1 (length-prefixed frames) lets us replace MSG_PEEK with "read full frame, parse strict, dispatch from parsed `mode`" — much cleaner.  **DONE 2026-05-03 (commit `45e8390d4`, mqc-2 P1)** — `mqc_accept_auto` rewritten to read the first length-prefixed frame, strict-parse it, and dispatch on the parsed `mode` field. |
